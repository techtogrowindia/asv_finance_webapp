...
/** Group number is the 3rd segment of the branch.center.group.member id. */
...
export function SavingsLedgerReport({ branchId }: { branchId?: string } = {}) {
  ...
  const groups = [...new Set((accounts ?? []).map((a) => groupOf(a.displayId)).filter(Boolean))].sort();
  const visible = (accounts ?? []).filter(
    (a) =>
      (!groupNo || groupOf(a.displayId) === groupNo) &&
      (!q.trim() || a.clientName.toLowerCase().includes(q.trim().toLowerCase()) || a.displayId.includes(q.trim())),
  );
      ...
      <div className="form-card" style={{ maxWidth: 'none', marginBottom: 18 }}>
        <div className="form-grid">
          ...
          <div className="field">
            ...
            <select className="input" value={groupNo} disabled={!centerId || groups.length === 0} onChange={(e) => setGroupNo(e.target.value)}>
          ...
          <div className="field">
            ...
            <input className="input" value={q} disabled={!centerId} placeholder="Search name or client ID" onChange={(e) => setQ(e.target.value)} />
          </div>
        </div>
      </div>
          ...
          <div className="panel-body">
            <div className="table-wrap" style={{ boxShadow: 'none', border: 'none' }}>
              <table className="data">
                <thead><tr><th>Client ID</th><th>Branch</th><th>Center</th><th>Member</th><th>Savings A/c</th><th>Start Date</th><th>No. of Dues</th><th>Closed Date</th><th>Balance</th><th></th></tr></thead>
                <tbody>
                  {visible.map((a) => (
                    <tr key={a.loanId}>
                  ...
                  {accounts && visible.length === 0 && <tr><td colSpan={10} className="empty">No savings accounts match.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
